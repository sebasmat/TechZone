...
const ReviewAndScore: NextPageWithLayout = () => {
  ...
  const findReview = async (id: string | string[]) => {
    const result = await axios
      .get(
        `https://tech-zone-api-n786.onrender.com/review?user=${user.UserFromDb.id}&product=${id}`
      )
      .then((data) => {
  ...
  const handleButtonRate = async (
    ...
    if (reviewFromDb.id == "") {
      try {
        axios
          .post(`https://tech-zone-api-n786.onrender.com/addreview`, {
            review: inputReview,
            score: fullStar,
          ...
          .then((data) => {
            setReviewFromDb({ ...reviewFromDb, id: data.data.id });
          });
